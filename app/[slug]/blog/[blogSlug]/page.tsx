...
export async function generateMetadata({ params }: Context): Promise<Metadata> {
  ...
  const { data: blog } = await supabase
    ...
    .select('baslik, icerik, kapak_url, psychologists!inner(booking_slug)')
    .eq('slug', blogSlug)
    .eq('yayinda', true)
    .single()
  ...
  return {
    ...
    openGraph: blog.kapak_url ? { images: [{ url: blog.kapak_url }] } : undefined,
  }
}
...
export default async function BlogDetayPage({ params }: Context) {
  ...
  const { data: psych } = await supabase
    .from('psychologists')
    .select('id, full_name, unvan, booking_slug, foto_url, bio_text, subscription_status, tema')
  ...
  const shareUrl = `https://seansify.com/${slug}/blog/${blogSlug}`
  ...
  return (
    <div style={{ background: C.pageBg, minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '40px 20px 60px' }}>
        ...
        {/* Geri */}
        <Link href={`/${slug}/blog`} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600, color: C.accentColor, textDecoration: 'none', marginBottom: 28 }}>
        ...
        {/* Makale kartı */}
        <article style={{ background: '#fff', borderRadius: 24, border: '1px solid rgba(0,0,0,0.07)', boxShadow: '0 4px 24px rgba(0,0,0,0.07)', overflow: 'hidden' }}>

          {/* Kapak resmi */}
          {blog.kapak_url && (
            <div style={{
              width: '100%', height: 260,
              background: `url(${blog.kapak_url}) center/cover no-repeat`,
            }} />
          )}
          ...
          <div style={{ padding: '32px 36px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
              ...
              <CopyLinkButton url={shareUrl} accentColor={C.accentColor} />
            </div>
            ...
            <div style={{
              ...
              <Link href={`/${slug}`} style={{ textDecoration: 'none', flexShrink: 0 }}>
                <div style={{
                  ...
                  background: `linear-gradient(135deg,${C.accentColor},${C.btnBg})`,
              ...
              <Link
                href={`/book/${slug}`}
                style={{
                  background: C.btnBg, color: '#fff', borderRadius: 10, padding: '9px 18px',
        ...
        {(onceki || sonraki) && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 24 }}>
            <div>
              {sonraki && (
                <Link href={`/${slug}/blog/${sonraki.slug}`} style={{ textDecoration: 'none', display: 'block', background: '#fff', borderRadius: 16, padding: '16px 20px', border: '1px solid rgba(0,0,0,0.07)' }}>
            ...
            <div>
              {onceki && (
                <Link href={`/${slug}/blog/${onceki.slug}`} style={{ textDecoration: 'none', display: 'block', background: '#fff', borderRadius: 16, padding: '16px 20px', border: '1px solid rgba(0,0,0,0.07)', textAlign: 'right' }}>
