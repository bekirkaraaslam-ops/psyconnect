...
export async function POST(_req: NextRequest, { params }: Context) {
  ...
  const { data: apt, error: fetchError } = await supabase
    .from('appointments')
    .select('id, psychologist_id, patient:patients(name_surname, phone_number)')
    .eq('id', id)
    ...
    if (waUrl && waKey) {
      await fetch(`${waUrl}/send`, {
        ...
        body: JSON.stringify({
          ...
          message:
            'Merhaba, maalesef talep ettiğiniz saatte beklenmedik bir yoğunluk oluşmuştur. Randevunuzu iptal etmek durumunda kaldık, anlayışınız için teşekkür ederiz.',
        }),
      }).catch(() => {})
    }
  }
