...
import AnamnezAiOzet from '@/components/patients/AnamnezAiOzet'
...
export default async function PatientDetailPage({ params, searchParams }: Props) {
  ...
  function sd(val: string | null | undefined) {
    if (!val) return ''
    try { return decryptNote(val) } catch { return '' }
  }

  const anamnezData = anamnezForm?.filled_at ? {
    filled_at:      anamnezForm.filled_at,
    sikayet:        sd(anamnezForm.sikayet_encrypted),
    sure:           sd(anamnezForm.sure_encrypted),
    gecmis_tedavi:  sd(anamnezForm.gecmis_tedavi_encrypted),
    ilac:           sd(anamnezForm.ilac_kullanim_encrypted),
    aile:           sd(anamnezForm.aile_gecmis_encrypted),
    uyku:           sd(anamnezForm.uyku_durum_encrypted),
    beslenme:       sd(anamnezForm.beslenme_durum_encrypted),
    acil_kisi:      sd(anamnezForm.acil_kisi_encrypted),
    ek_notlar:      sd(anamnezForm.ek_notlar_encrypted),
  } : null

  return (
    <div className="flex-1">
      <Topbar title={patient.name_surname} backHref="/patients" />

      <div className="p-3 md:p-6" style={{ maxWidth: anamnezForm?.filled_at ? '1100px' : '896px' }}>
        <div className={anamnezForm?.filled_at ? 'flex gap-5 items-start' : ''}>
          ...
          {/* Sağ — AI özet paneli (sadece anamnez doluysa) */}
          {anamnezForm?.filled_at && anamnezData && (
            <div className="hidden md:block shrink-0" style={{ width: '260px', position: 'sticky', top: '24px' }}>
              <AnamnezAiOzet formId={anamnezForm.id} anamnezData={anamnezData} />
            </div>
          )}
